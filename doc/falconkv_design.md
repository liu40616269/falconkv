...
| **Store** | SSD 数据持久化、自管理空间（BuddyAllocator）、本地元数据索引（StoreMetaIndex）、驱逐管理、元数据同步（MetaSyncClient） | 独立进程或同进程 | brpc RPC / in-process |
...
│  │BuddyAlloc │  │BuddyAlloc │   │BuddyAlloc │              │
...
│  ┌──────────────────────────────────────────────┐  │
│  │ FalconKV Store                               │  │
│  │ BuddyAllocator + StoreMetaIndex + MetaSyncCli │  │
│  │ ┌──────────────────────────────────────────┐ │  │
...
- **Store 自管理**：每个 Store 独立管理 BuddyAllocator、StoreMetaIndex、驱逐策略，通过 MetaSyncClient 异步同步到 Meta
...
- **空间回收**：Buddy 合并减少外部碎片
- **位置**：BuddyAllocator 位于 `src/common/`，被 Store 和 Meta（测试用）共同使用
...
Store.Put(key, data, size):
  1. BuddyAllocator.AllocChunk() → offset    // 分配 SSD 空间
...
Store 驱逐 (后台线程):
  ...
  4. PendingEvictQueue 后台线程:
     超过 5s 宽限期 → BuddyAllocator.FreeChunk  // 真正回收空间
```
...
T3: BuddyAllocator.FreeChunk + Index.Remove  ← 空间回收
```
...
| BatchPut (100 keys) | < 10ms | 本地 Store BuddyAlloc + DirectIO 写入 |
