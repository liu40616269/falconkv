...
3. 当前Store写数据前分配空间固定使用chunk_size, 而不是数据的真实大小, 请按照数据的真实大小进行空间分配; 数据Evict时也需要基于真实Size进行回收
   同时请检查代码chunk_size是否真的有用, 如无用请删除chunck_size相关的配置和代码
4. capacity_bytes 应该归属在Store模块, 不应该在Client模块, 请更新falconkv.json配置文件
   同时Store模块应该要由node_id字段用于标志Store归属的节点
