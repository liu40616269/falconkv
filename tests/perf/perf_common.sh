#!/usr/bin/env bash
#
# perf_common.sh — Shared functions for FalconKV performance tests.
#
# This file is meant to be sourced by run_perf_*.sh wrapper scripts.
...
_PROJECT_ROOT="$(cd "${_PERF_COMMON_DIR}/../.." && pwd)"
_BUILD_DIR="${_PROJECT_ROOT}/build"

FALCONKV_MASTER="${_BUILD_DIR}/src/meta/falconkv_master"
FALCONKV_SCHED="${_BUILD_DIR}/src/scheduler/falconkv_sched"
PERF_DIR="${_PERF_COMMON_DIR}"
...
# ---------------------------------------------------------------------------
# Global PIDs for cleanup (prefixed with _ to avoid collision)
# ---------------------------------------------------------------------------
_META_PID=""
_SCHED_PID=""
_CLIENT_PIDS=""
...
_perf_cleanup() {
    ...
    for pid in $_CLIENT_PIDS; do
        kill "$pid" 2>/dev/null || true
    done
    # Stop scheduler
    if [ -n "$_SCHED_PID" ]; then
        stop_process "$_SCHED_PID" "Scheduler"
    fi
    # Stop meta
    if [ -n "$_META_PID" ]; then
        stop_process "$_META_PID" "Meta"
    fi
    # Clean up UDS file
    if [ -n "${_scheduler_uds_path:-}" ] && [ -e "${_scheduler_uds_path}" ]; then
        rm -f "${_scheduler_uds_path}"
    fi
    echo "Cleanup done."
}

# ---------------------------------------------------------------------------
# Main entry point: run_perf <config_file> <test_name>
#
# Orchestrates the full benchmark lifecycle:
#   1. Parse config
#   2. Create directories
#   3. Start Meta -> wait for port
#   4. Start Scheduler -> wait for UDS  (optional)
#   5. Launch perf_client.py for each client in parallel
#   6. Wait for all clients
#   7. Stop Meta / Scheduler
#   8. Run perf_aggregate.py
# ---------------------------------------------------------------------------
run_perf() {
    local CONFIG_FILE="$1"
    local TEST_NAME="${2:-FalconKV Performance Test}"
    ...
    # Store UDS path for cleanup
    _scheduler_uds_path="${scheduler_uds_path}"
